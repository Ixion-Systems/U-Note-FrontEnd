...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Save token securely based on keepSession preference
      if (formData.keepSession) {
    ...
    <div ref={containerRef} className="w-full min-h-screen flex flex-col md:flex-row relative z-10 overflow-hidden">
      ...
      <div className="orange-stripe w-full md:w-1/3 bg-[#FF6B00] min-h-screen flex flex-col items-center justify-center p-4 md:p-8 relative z-20 border-b-[3px] md:border-b-0 md:border-r-[3px] border-black shadow-[10px_0px_20px_rgba(0,0,0,0.5)]">
        ...
        <div className="form-content w-full max-w-[450px] bg-[#1A1A1A] p-8 md:p-10 rounded-2xl border-[3px] border-black shadow-[8px_8px_0px_#000000]">
          <h2 className="font-headline-lg text-[40px] md:text-[50px] uppercase text-white mb-8 leading-none tracking-tight">INICIAR SESIÓN</h2>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-8">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mt-2">
              ...
              <a href="#" className="font-body-md text-[#FF6B00] text-sm hover:underline hover:text-white transition-colors">¿Olvidaste tu contraseña?</a>
            </div>

            <button 
              type="submit" 
              disabled={loading}
              className="mt-4 w-full bg-[#FF6B00] text-black font-headline-sm text-[24px] py-4 border-[3px] border-black hover:bg-white hover:-translate-y-1 hover:shadow-[6px_6px_0px_#FFFFFF] transition-all uppercase tracking-widest disabled:opacity-50 disabled:hover:translate-y-0 disabled:hover:shadow-none"
            >
          ...
          <p className="font-body-md text-white/60 text-center mt-10 text-sm">
            ¿No tienes cuenta? <Link to="/signup" className="text-white hover:text-[#FF6B00] hover:underline transition-colors font-bold">Regístrate</Link>
          </p>
        </div>
      </div>
      ...
      <div className="w-full md:w-2/3 flex items-center justify-center p-8 md:p-16 relative z-10">
        <h1 className="font-headline-xl text-[12vw] md:text-[8vw] leading-[0.85] uppercase text-black max-w-[1200px] text-center md:text-left drop-shadow-md">
          ...
          <br />
          <span className="relative inline-block overflow-hidden py-1">
            <span className="mask-block bg-[#FF6B00] absolute inset-0 z-10 origin-bottom"></span>
            <span className="text-[#FF6B00] drop-shadow-[2px_2px_0px_#000000]">MATERIAS</span>
          </span>
          <br />
          <span className="relative inline-block overflow-hidden py-1">
            <span className="mask-block bg-black absolute inset-0 z-10 origin-bottom"></span>
